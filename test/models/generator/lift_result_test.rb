...
class LiftResultTest < ActiveSupport::TestCase
  ...
  test "succeed when attempt is 0 and result is 1" do
    result = Generator::LiftResult.new(attempt: 0, result: 1)

    assert result.success?
  end

  test "succeed when attempt is 0 and result is 2" do
    result = Generator::LiftResult.new(attempt: 0, result: 2)

    assert result.success?
  end

  test "fail when attempt is 0 and result is 3" do
    result = Generator::LiftResult.new(attempt: 0, result: 3)

    assert_not result.success?
  end

  test "succeed when attempt is 1 and result is 0" do
    result = Generator::LiftResult.new(attempt: 1, result: 0)

    assert result.success?
  end

  test "succeed when attempt is 1 and result is 1" do
    result = Generator::LiftResult.new(attempt: 1, result: 1)

    assert result.success?
  end

  test "fail when attempt is 1 and result is 2" do
    result = Generator::LiftResult.new(attempt: 1, result: 2)

    assert_not result.success?
  end

  test "fail when attempt is 1 and result is 3" do
    result = Generator::LiftResult.new(attempt: 1, result: 3)

    assert_not result.success?
  end

  test "succeed when attempt is 2 and result is 0" do
    result = Generator::LiftResult.new(attempt: 2, result: 0)

    assert result.success?
  end

  test "fail when attempt is 2 and result is 1" do
    result = Generator::LiftResult.new(attempt: 2, result: 1)

    assert_not result.success?
  end

  test "fail when attempt is 2 and result is 2" do
    result = Generator::LiftResult.new(attempt: 2, result: 2)

    assert_not result.success?
  end

  test "fail when attempt is 2 and result is 3" do
    result = Generator::LiftResult.new(attempt: 2, result: 3)

    assert_not result.success?
  end

  test "generate result between 0 and 3" do
    lift_result = Generator::LiftResult.new(attempt: 1)
    expected_range = (0..3)
