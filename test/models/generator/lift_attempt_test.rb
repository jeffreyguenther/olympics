...
class LiftAttemptTest < ActiveSupport::TestCase
  test "lift is successful" do
    attempt_num = 0
    result = Generator::LiftResult.new(attempt: attempt_num, result: 0)
    attempt = Generator::LiftAttempt.new(attempt: attempt_num, weight: 100, result: result)
  ...
  test "lift is not successful" do
    attempt_num = 0
    result = Generator::LiftResult.new(attempt: attempt_num, result: 3)
    attempt = Generator::LiftAttempt.new(attempt: attempt_num, weight: 100, result: result)
  ...
  test "#next_weight is increment 5lbs on success" do
    ...
    expected_weight = 105
    result = Generator::LiftResult.new(attempt: attempt_num, result: 0)
    attempt = Generator::LiftAttempt.new(attempt: attempt_num, weight: attempted_weight, result: result)
  ...
  test "result of attempt is random" do
    attempt_num = 0
    attempted_weight = 100
    attempt = Generator::LiftAttempt.new(attempt: attempt_num, weight: attempted_weight)
